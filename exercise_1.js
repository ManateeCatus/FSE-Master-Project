...
function draw1(){
  ...
  if (ceil(((endTime - timer) / 60)) % 60 > 9) {
    text(floor(ceil((endTime - timer) / 60) / 60) + ':' +ceil(((endTime - timer) / 60))     % 60, 340, 30);
      
    }
  else {
     text(floor(ceil((endTime - timer) / 60) / 60) + ':0' +ceil(((endTime - timer)            /60))% 60, 340, 30); 
    }
 ...
 return{setup1,draw1,mousePressed1,doubleClicked1} 
}

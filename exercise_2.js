  ...
  if (gameRunning)
{
  ...
  if (ceil(((endTime - timer) / 60)) % 60 > 9)
    {
  text(floor(ceil((endTime - timer) /60) / 60) + ':' +ceil(((endTime - timer) / 60)) % 60, widthC - 70, 30);
      
    }
  else
    {
     text(floor(ceil((endTime - timer) / 60) / 60) + ':0' +ceil(((endTime - timer) / 60)) % 60, widthC - 70, 30); 
  ...
  return{setup2,draw2,mousePressed2,doubleClicked2};
}